...
class Generator {
        ...
        var $parseReplaceObjects = array();
        function register_object_recursive($name,&$allobjs) {
                        global	$matcher;
            //echo "ATTEMPT: $name\n";
            if (!isset($allobjs[$name])) {
                return;
            }
            $object = $allobjs[$name];
            
            if (!isset($object->parent)) {
                // fire away and register it...
                // we dont care!!!
               // echo "JUST REGISTER\n";
                $matcher->register_object($name, false);
                $this->parseReplaceObjects[$name] = $object;
                unset($allobjs[$name]);
                return;
            }
            
            $parentName = $object->parent[1] . $object->parent[0];
           // echo "NEED PARENT: $parentName\n";
            if (isset($allobjs[$parentName])) {
                // recurse!!!!
                $this->register_object_recursive($parentName,$allobjs);
            }
            $fullname = $object->in_module . $object->name;
            if (!isset($this->is_gtk_object[$fullName]) && 
                isset($this->is_gtk_object[$parentName] )) {
                    $this->is_gtk_object[$fullname] = $this->is_gtk_object[$parentName];
            }
            ///now finally register it..
             $matcher->register_object($name,$this->is_gtk_object[$fullname] );
             $this->parseReplaceObjects[$name] = $object;
            // clear it
            unset($allobjs[$name]);
        ...
        function register_types($parser = null)
        {
                ...
                // we must register objects in order..
                $allobjs = array();
                foreach ($parser->objects as $object) {
                    $allobjs[$object->c_name]  = $object;
                }
                //print_r($allobjs);
                foreach(array_keys($allobjs) as $name) {
                    $this->register_object_recursive($name,$allobjs);
                }
                
                
                // so first make a quick map of parent to child...
        ...
        function write_structs($fp)
        {
                ...
                foreach ($this->parser->structs as $struct) {
                        if ($this->overrides->is_ignored($struct->c_name)) {
                            continue;
                        }
        ...
        function write_objects($fp)
        {
                ...
                foreach ($this->parseReplaceObjects as $name=>$object) {
                        ...
                        */
                        ...
                        foreach ($this->parser->find_methods($object) as $method) {
                                if ($this->overrides->is_overriden($method->c_name)) {
                                        ...
                                        //echo "TEST: {$object->in_module}{$object->name}\n";
                                        //print_r($this->is_gtk_object);
                                        if ($this->write_method(
                                                $fp, 
                                                $object->c_name,
                                                $this->is_gtk_object[$object->in_module . $object->name],
                                                $method
                                            )) 
                                        {
                                                $functions_decl .= sprintf($function_entry_tpl,
