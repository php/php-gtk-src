...
$(srcdir)/gen_gdkpixbuf.c: $(srcdir)/gdkpixbuf.defs $(srcdir)/gdkpixbuf.overrides
	$(PHP) -f $(top_srcdir)/generator/generator.php -- -o $(srcdir)/gdkpixbuf.overrides -p gdk_pixbuf -c GdkPixbuf -r $(top_srcdir)/ext/gtk%2b/gtk.defs -r $(top_srcdir)/ext/gtk%2b/gdk.defs $(srcdir)/gdkpixbuf.defs > $(srcdir)/gen_gdkpixbuf.c
