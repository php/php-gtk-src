dnl $Id$
dnl config.m4 for gdkpixbuf module
...
PHP_GTK_ARG_ENABLE(gdkpixbuf,for GdkPixbuf support,
[  --enable-gdkpixbuf      Enable GdkPixbuf support])

if test "$PHP_GTK_GDKPIXBUF" != "no"; then
  PHP_PATH_GDK_PIXBUF(0.11.0,have_gdkpixbuf=yes,have_gdkpixbuf=no)
  if test "$have_gdkpixbuf" != "yes"; then
    AC_MSG_ERROR(Unable to locate gdk-pixbuf)
  else
