...
dnl Check for glib
AM_PATH_GLIB_2_0(glib_required_version,,[AC_MSG_ERROR(PHP-GTK 2.x requires GLib glib_required_version)],gobject)
PHP_EVAL_INCLINE($GLIB_CFLAGS)
PHP_EVAL_LIBLINE($GLIB_LIBS, PHP_GTK2_SHARED_LIBADD)

dnl Check for Gtk+
AM_PATH_GTK_2_0(gtk_required_version,,[AC_MSG_ERROR(PHP-GTK 2.x requires GTK+ gtk_required_version)])
